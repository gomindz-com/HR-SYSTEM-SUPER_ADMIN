...
export default function DashboardPage() {
  const { companyStats, statsLoading, fetchCompanyStats } = useSuperAdminStore()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <MetricCard title="Active Subscriptions" value={metrics?.companiesWithActiveSubscription || 0} icon={Users} color="emerald" />
        <MetricCard title="Trial Subscriptions" value={metrics?.companiesWithTrialSubscription || 0} icon={Clock} color="amber" />
        <MetricCard title="Expired Subscriptions" value={metrics?.companiesWithExpiredSubscription || 0} icon={XCircle} color="rose" />
        <MetricCard title="Lifetime Access" value={metrics?.companiesWithLifetimeAccess || 0} icon={Infinity} color="violet" />
        <MetricCard title="Pending Subscriptions" value={metrics?.companiesWithPendingSubscription || 0} icon={CreditCard} color="teal" />
      </div>
          ...
          <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">
                  {metrics?.totalCompanies
                    ? Math.round((metrics.companiesWithActiveSubscription / metrics.totalCompanies) * 100)
                    : 0}
                  %
                </span>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">
                  {metrics?.totalCompanies
                    ? Math.round((metrics.companiesWithTrialSubscription / metrics.totalCompanies) * 100)
                    : 0}
                  %
                </span>
                <TrendingUp className="h-4 w-4 text-chart-2" />
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">
                  {metrics?.totalCompanies
                    ? Math.round((metrics.companiesWithExpiredSubscription / metrics.totalCompanies) * 100)
                    : 0}
                  %
                </span>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium">{metrics?.companiesWithActiveSubscription || 0}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Trial Companies</span>
              <span className="text-sm font-medium">{metrics?.companiesWithTrialSubscription || 0}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Lifetime Access</span>
              <span className="text-sm font-medium">{metrics?.companiesWithLifetimeAccess || 0}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Pending</span>
              <span className="text-sm font-medium">{metrics?.companiesWithPendingSubscription || 0}</span>
            </div>
          </CardContent>
        </Card>
      </div>
